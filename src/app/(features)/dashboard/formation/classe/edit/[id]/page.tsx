...
export default function EditClassPage() {
  ...
  const params = useParams(); // Next.js App Router param
  ...
  const fetchClasse = async () => {
    ...
    setLoading(true);
    try {
      const data = await ClasseService.getClasse(classId);
      setFormData({
        name: data.name,
        level: data.level?.toString() || "",
        academic_year: data.academic_year,
        formation_id: data.formation_id?.toString() || "",
        max_students: data.max_students?.toString() || "",
        is_active: data.is_active,
      });
    } catch (err: any) {
      setApiError(err?.response?.data?.message || "Erreur lors de la récupération de la classe");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      await ClasseService.updateClasse(classId, {
        name: formData.name,
      ...
      <div className="border-b border-gray-200 bg-white">
        <div className="px-8 py-6 flex items-center justify-between">
          <div className="flex items-center">
            ...
            <div>
              <h1 className="text-2xl font-poppins font-semibold text-gray-900">Modifier la Classe</h1>
              <p className="text-sm font-poppins text-gray-600 mt-1">Modifier les informations de la classe académique</p>
            </div>
          </div>
