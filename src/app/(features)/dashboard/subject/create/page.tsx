...
export default function SubjectCreationForm() {
  ...
  // Options pour les crédits (de 1 à 10)
  const creditOptions = Array.from({length: 10}, (_, i) => ({
    value: (i + 1).toString(),
    label: `${i + 1} crédit${i > 0 ? 's' : ''}`
  }));

  // Options pour les coefficients (de 1 à 5)
  const coefficientOptions = Array.from({length: 5}, (_, i) => ({
    value: (i + 1).toString(),
    label: `Coefficient ${i + 1}`
  }));

  // Simulation de récupération des formations
  const fetchFormations = async () => {
    try {
      setLoadingFormations(true);
      // Simulation d'appel API - remplacer par votre service
      setTimeout(() => {
        const mockFormations = [
          { value: "1", label: "Licence Informatique" },
          { value: "2", label: "Master Design UX/UI" },
          { value: "3", label: "Licence Mathématiques" },
          { value: "4", label: "Master Intelligence Artificielle" },
        ];
        setFormationOptions(mockFormations);
  ...
  // Gestion des selects
  const handleSelectChange = (field: string) => (e: React.ChangeEvent<HTMLSelectElement> | { target: { value: string } }) => {
    const value = e.target.value;
    setFormData(prev => ({ ...prev, [field]: value }));
    if (errors[field]) setErrors(prev => ({ ...prev, [field]: "" }));
  };
      ...
      <div className="border-b border-gray-200 bg-white">
        <div className="px-8 py-6 flex items-center justify-between">
          <div className="flex items-center">
            ...
            <div>
              <h1 className="text-2xl font-poppins font-semibold text-gray-900">Nouvelle Matière</h1>
        ...
        <div className="max-w-4xl mx-auto h-full bg-white border border-gray-200 rounded-lg overflow-hidden">
          ...
          <div className="p-8 border-b border-gray-200">
            <div className="flex items-center mb-6">
              ...
              <h2 className="text-lg font-poppins font-medium text-gray-900">Informations générales</h2>
            </div>
          ...
          <div className="p-8 border-b border-gray-200">
            <div className="flex items-center mb-6">
              ...
              <h2 className="text-lg font-poppins font-medium text-gray-900">Configuration académique</h2>
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <SelectInput
                label="Formation"
                placeholder={loadingFormations ? "Chargement..." : "Sélectionner une formation"}
          ...
          <div className="p-8 border-b border-gray-200">
            <div className="flex items-center mb-6">
              ...
              <h2 className="text-lg font-poppins font-medium text-gray-900">Évaluation et pondération</h2>
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <SelectInput
              ...
              <div className="flex items-center pt-8">
                ...
                />
                <label htmlFor="is_active" className="ml-3 text-sm text-gray-700 font-poppins">
                  Matière active
                </label>
              </div>
            </div>
